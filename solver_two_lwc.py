...
for the Time-Dependent Hump Problem

   du/dt - eps*Lap(u) + b.grad(u) + c*u = f,   in (0,1)^2
   u = 0 on boundary,   u(0,.) = 0
...
#parameters['form_compiler']['quadrature_degree'] = 11
...
print("=" * 70)
print("HUMP PROBLEM — SUPG + YZbeta (Pure UFL)")
print("=" * 70)
print("  epsilon = %e" % epsilon)
print("  b = (%g, %g)" % (b1_val, b2_val))
print("  c = %g" % c_val)
...
# =============================================================================
nx_mesh = 64
mesh = UnitSquareMesh(nx_mesh, nx_mesh, "left")
...
def analytical_solution_np(t, x1, x2):
    ...
    alpha_val = 2.0 / np.sqrt(epsilon)
    g = 0.0625 - (x1 - 0.5)**2 - (x2 - 0.5)**2
    phi = 0.5 + np.arctan(alpha_val * g) / np.pi
    p = x1 * (1.0 - x1) * x2 * (1.0 - x2)
    return 16.0 * np.sin(np.pi * t) * p * phi


# =============================================================================
# SOURCE TERM & EXACT SOLUTION — PURE UFL
#
# u_exact = 16 sin(pi t) S(x)
# S(x) = p(x) phi(x)
# p = x1(1-x1) x2(1-x2)
# phi = 0.5 + atan(alpha g)/pi,  alpha = 2/sqrt(eps)
# g = 0.0625 - (x1-0.5)^2 - (x2-0.5)^2
#
# f = du/dt - eps Lap(u) + b.grad(u) + c u
#   = 16 pi cos(pi t) S
#     + 16 sin(pi t) [-eps Lap(S) + b.grad(S) + c S]
#
# UFL computes grad(S) and div(grad(S)) automatically!
# =============================================================================
...
pi_c    = Constant(np.pi)
alpha_c = Constant(2.0 / np.sqrt(epsilon))

# g(x)
g_ufl = Constant(0.0625) - (x1_ufl - 0.5)**2 - (x2_ufl - 0.5)**2

# phi(x) = 0.5 + atan(alpha*g) / pi
phi_ufl = 0.5 + atan(alpha_c * g_ufl) / pi_c

# p(x)
p_ufl = x1_ufl * (1.0 - x1_ufl) * x2_ufl * (1.0 - x2_ufl)

# S(x) = p * phi
S_ufl = p_ufl * phi_ufl

# UFL auto-differentiation for grad(S) and Laplacian(S)
grad_S = grad(S_ufl)       # UFL computes this symbolically
lap_S  = div(grad(S_ufl))  # = div(grad(S)) = Laplacian(S)

# Time-dependent multipliers (Constant, updated each step)
sin_pit = Constant(0.0)
cos_pit = Constant(0.0)

# f(t,x) = 16 pi cos(pi t) S + 16 sin(pi t) [-eps Lap(S) + b.grad(S) + c S]
f_ufl = (Constant(16.0) * pi_c * cos_pit * S_ufl
         + Constant(16.0) * sin_pit * (- eps_c * lap_S
                                         + dot(b_ufl, grad_S)
                                         + c_c * S_ufl))

# Exact solution in UFL (for error computation)
u_exact_ufl = Constant(16.0) * sin_pit * S_ufl
...
shock_visc = conditional(gt(shock_raw, shock_cap), shock_cap, shock_raw)
...
u_prev_snapshot    = None  # solution at t_final - dt (for PINN PDE residual)
t = 0.0
step = 0
...
while t < t_final - 1e-14:
    ...
    # ---- Update time (pure Constant assignment, no DOF issues) ----
    sin_pit.assign(np.sin(np.pi * t))
    cos_pit.assign(np.cos(np.pi * t))
...
# Global color scale
u_anal_peak = analytical_solution_np(0.5, X_plot, Y_plot)
global_vmax = u_anal_peak.max()
...
fig.suptitle('SUPG+YZbeta FEM Solution', fontsize=14, y=1.01)
plt.tight_layout()
...
for idx in range(n_rows * n_cols):
    ...
    if idx < n_snaps:
        ...
        ax.set_visible(False)
    ax.set_xticks([]); ax.set_yticks([]); ax.set_aspect('equal')
fig.suptitle('Analytical Solution', fontsize=14, y=1.01)
plt.tight_layout()
...
for idx in range(n_rows * n_cols):
    ...
    if idx < n_snaps:
        ...
        ax.set_visible(False)
    ax.set_xticks([]); ax.set_yticks([]); ax.set_aspect('equal')
fig.suptitle('|FEM - Analytical| Error', fontsize=14, y=1.01)
plt.tight_layout()
...
ax.set_title('$L^2$ Error (SUPG + YZbeta)', fontsize=14)
...
#ax.set_title('Cross-Section at $x_2 = 0.5$', fontsize=14)
...
for idx_p, (data, title) in enumerate([
        ...
        (u_fem_end, 'SUPG+YZbeta FEM (t=%.2f)' % t_end)]):
    ax = fig.add_subplot(1, 2, idx_p + 1, projection='3d')
...
print("  Terminal L2:     %.6e" % l2_errors_all[-1])
...
# =============================================================================
class ComputeRHS:
    """Compute source term f for the PDE in numpy (for PINN training).
    Supports vectorized t_val for multi-time collocation."""
    def __init__(self, eps, b, c):
        self.eps = eps
        self.alpha = 2.0 / np.sqrt(eps)
        self.b1, self.b2 = b
        self.c = c

    def _spatial_derivatives(self, x1, x2):
        x1 = np.asarray(x1, dtype=np.float64)
        x2 = np.asarray(x2, dtype=np.float64)
        a = self.alpha
        g = 0.0625 - (x1 - 0.5)**2 - (x2 - 0.5)**2
        gx1 = -2.0 * (x1 - 0.5)
        gx2 = -2.0 * (x2 - 0.5)
        gx1x1 = -2.0 * np.ones_like(x1)
        gx2x2 = -2.0 * np.ones_like(x2)
        ag = a * g
        denom = 1.0 + ag**2
        phi = 0.5 + np.arctan(ag) / np.pi
        phi_x1 = (a / (np.pi * denom)) * gx1
        phi_x2 = (a / (np.pi * denom)) * gx2
        phi_x1x1 = (a / np.pi) * (gx1x1 / denom - 2.0 * a**2 * g * gx1**2 / denom**2)
        phi_x2x2 = (a / np.pi) * (gx2x2 / denom - 2.0 * a**2 * g * gx2**2 / denom**2)
        p = x1 * (1.0 - x1) * x2 * (1.0 - x2)
        px1 = (1.0 - 2.0 * x1) * x2 * (1.0 - x2)
        px2 = x1 * (1.0 - x1) * (1.0 - 2.0 * x2)
        px1x1 = -2.0 * x2 * (1.0 - x2)
        px2x2 = -2.0 * x1 * (1.0 - x1)
        S = p * phi
        Sx1 = px1 * phi + p * phi_x1
        Sx2 = px2 * phi + p * phi_x2
        Sx1x1 = px1x1 * phi + 2.0 * px1 * phi_x1 + p * phi_x1x1
        Sx2x2 = px2x2 * phi + 2.0 * px2 * phi_x2 + p * phi_x2x2
        laplacian_S = Sx1x1 + Sx2x2
        return S, Sx1, Sx2, laplacian_S

    def compute_f(self, t_val, x1, x2):
        """Compute f(t, x1, x2). All inputs can be arrays."""
        S, Sx1, Sx2, lap_S = self._spatial_derivatives(x1, x2)
        t_val = np.asarray(t_val, dtype=np.float64)
        sin_t = np.sin(np.pi * t_val)
        cos_t = np.cos(np.pi * t_val)
        f = (16.0 * np.pi * cos_t * S
             + 16.0 * sin_t * (-self.eps * lap_S
                               + self.b1 * Sx1 + self.b2 * Sx2
                               + self.c * S))
        return f
...
# BC points: (t, x₁, x₂) on ∂Ω at several times
n_bc_per_side = 40
n_bc_times = 3
...
for t_bc in bc_t_vals:
    for side in range(4):
        if side == 0:
            xx = torch.linspace(0, 1, n_bc_per_side, device=device)
            yy = torch.zeros_like(xx)
        elif side == 1:
            yy = torch.linspace(0, 1, n_bc_per_side, device=device)
            xx = torch.ones_like(yy)
        elif side == 2:
            xx = torch.linspace(1, 0, n_bc_per_side, device=device)
            yy = torch.ones_like(xx)
        else:
            yy = torch.linspace(1, 0, n_bc_per_side, device=device)
            xx = torch.zeros_like(yy)
        tt = torch.full_like(xx, t_bc)
        bc_points_list.append(torch.stack([tt, xx, yy], dim=1))
...
def train_hybrid_pinn():
    ...
    def hybrid_loss(model, data_x, data_y, pde_pts, bc_pts, epoch):
        # --- Data fidelity loss (multi-snapshot FEM data) ---
        pred = model(data_x)
        ...
        if pde_pts is not None and len(pde_pts) > 0:
            try:
        ...
        if bc_pts is not None and len(bc_pts) > 0:
            bc_pred = model(bc_pts).squeeze()
            bc_loss = torch.mean(bc_pred ** 2)

        # --- Adaptive weight scheduling ---
        if epoch < 1000:
            w_data, w_pde, w_bc = 1.0, 0.05, 0.01
        elif epoch < 1500:
            w_data, w_pde, w_bc = 0.5, 0.1, 0.05
        else:
            w_data, w_pde, w_bc = 0.1, 0.5, 0.1
    ...
    dataset = TensorDataset(coords_tensor, u_fem_tensor.unsqueeze(1))
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_x, batch_y in dataloader:
            ...
            # PDE collocation: random (t, x₁, x₂) in time window × (0,1)²
            n_pde = 256
            ...
            total_loss, lc = hybrid_loss(model, batch_x, batch_y, pde_pts, bc_points, epoch)
            ...
            valid = all(not (torch.isnan(p.grad).any() or torch.isinf(p.grad).any())
        ...
        if avg['total'] < best_loss:
            best_loss = avg['total']
            torch.save(model.state_dict(), 'hybrid_pinn_terminal.pt')
...
vmin = min(u_anal_grid.min(), u_fem_grid.min(), u_pinn_grid.min())
vmax = max(u_anal_grid.max(), u_fem_grid.max(), u_pinn_grid.max())
levels = np.linspace(vmin - 1e-10, vmax + 1e-10, 25)

for ax, data, title in zip(
    axes, [u_anal_grid, u_fem_grid, u_pinn_grid],
    [f'Analytical (t={t_terminal:.2f})', f'SUPG+YZβ (t={t_terminal:.2f})',
     f'Hybrid PINN (t={t_terminal:.2f})']):
    im = ax.contourf(X_ev, Y_ev, data, levels=levels, cmap='plasma')
...
for ax, err, title in zip(axes, [err_fem, err_pinn],
    ['|SUPG+YZβ − Analytical|', '|PINN − Analytical|']):
...
ax.plot(x_ev, u_fem_line, 'g--', linewidth=2, label='SUPG-YZ$β$')
ax.plot(x_ev, u_pinn_line, 'r-.', linewidth=2, label='Hybrid PINN')
ax.set_xlabel('$x_1$', fontsize=14)
ax.set_ylabel(f'$u(0.5, x_1, 0.5)$', fontsize=14)
#ax.set_title(f'Cross-Section at $x_2 = 0.5$, $t = {t_terminal:.2f}$', fontsize=14)
ax.legend(fontsize=12); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('ex_three_terminal_cross_section_x2_05.png', dpi=400, bbox_inches='tight')
plt.close()
...
fig, ax = plt.subplots(figsize=(10, 6))
ax.semilogy(snapshot_times, l2_errors_all, 'g-', linewidth=1.5, label='SUPG-YZ$β$', alpha=0.8)
...
#ax.set_title('$L^2$ Error: FEM + PINN correction', fontsize=14)
ax.legend(fontsize=12); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('ex_three_l2_error_over_time_with_pinn.png', dpi=400, bbox_inches='tight')
plt.close()
...
for data, title, fname in [
    (u_anal_grid, f'Analytical (t={t_terminal:.2f})',    '3d_surface_analytical.png'),
    (u_fem_grid,  f'SUPG+YZβ FEM (t={t_terminal:.2f})', '3d_surface_fem.png'),
    (u_pinn_grid, f'Hybrid PINN (t={t_terminal:.2f})',   '3d_surface_pinn.png')]:
...
print(f"  L2(FEM)  = {l2_fem_terminal:.6e}")
...
print(f"  FEM:        {u_fem_center_t:.6e}  (err: {abs(u_anal_center - u_fem_center_t):.4e})")
...
print("  • 3d_surface_terminal.png           (3-panel 3D)")
print("=" * 70)